...
import com.util.easy.helper.PermissionHelper;
...
public class Easy {
    ...
    /**
     * Callback interface to receive the results of {@code Easy.request()}
     * calls.
     */
    ...
    public static boolean has(@NonNull Context context,
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            Log.w(TAG, "has: API version < M, returning true by default");
     ...
     * @see Manifest.permission
     */
    public static void request(@NonNull Activity host, @NonNull String rationale, int requestCode, @Size(min = 1) @NonNull String... perms) {
        request(new PermissionRequest.Builder(host, requestCode, perms).setRationale(rationale).build());
    }

    public static void request(@NonNull Activity host, @StringRes int rationale, int requestCode, @Size(min = 1) @NonNull String... perms) {
        request(new PermissionRequest.Builder(host, requestCode, perms).setRationale(host.getResources().getString(rationale)).build());
    }

    /**
     * Request permissions from a Support Fragment with standard OK/Cancel buttons.
     *
     * @see #request(Activity, String, int, String...)
     */
     ...
     * @param perms          a set of permissions to be requested.
     * @see Manifest.permission
     * @deprecated use {@link #request(PermissionRequest)} instead
     */
    @Deprecated
    public static void request(
     ...
     *
     * @see #request(Activity, String, int, int, int, String...)
     * @deprecated use {@link #request(PermissionRequest)} instead
     */
    @Deprecated
    public static void request(
    ...
    public static void request(PermissionRequest request) {
        ...
        if (has(request.getHelper().getContext(), request.getPerms())) {
     ...
     */
    public static void onResult(int requestCode,
    ...
    private static void notifyAlreadyHasPermissions(@NonNull Object object,
        ...
        onResult(requestCode, perms, grantResults, object);
    }
